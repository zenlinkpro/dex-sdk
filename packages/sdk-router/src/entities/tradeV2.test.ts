...
describe('TradeV2', () => {
  ...
  describe('#bestTradeExactIn', () => {
    ...
    it('provides best route', () => {
      const result = TradeV2.bestTradeExactIn(
        2023,
        [pair_0_2, pair_2_3, pair_2_4, pair_3_4],
        [stableSwap0, stableSwap1],
        new TokenAmount(token4, '1000000000000000000'),
      ...
      expect(result[0].route.routePath).toHaveLength(2);
      expect(result[0].route.tokenPath).toEqual([token4, token2, token1]);
      expect(result[0].inputAmount).toEqual(new TokenAmount(token4, '1000000000000000000'));
      expect(result[0].outputAmount).toEqual(new TokenAmount(token1, '1230291'));
      expect(result[1].route.routePath).toHaveLength(3);
      expect(result[1].route.tokenPath).toEqual([token4, token2, token0, token1]);
      expect(result[1].inputAmount).toEqual(new TokenAmount(token4, '1000000000000000000'));
      expect(result[1].outputAmount).toEqual(new TokenAmount(token1, '1229660'));
    });
    ...
    it('respects maxHops', () => {
      ...
      expect(result[0].route.routePath).toHaveLength(1);
      expect(result[0].route.tokenPath).toEqual([token4, token1]);
    });
  ...
  describe('#minimumAmountOut', () => {
    const exactIn = TradeV2.bestTradeExactIn(
      2023,
      [pair_0_2, pair_2_3, pair_2_4, pair_3_4],
      [stableSwap0, stableSwap1],
      new TokenAmount(token4, '1000000000000000000'),
