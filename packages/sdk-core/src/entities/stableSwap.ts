...
import { FOUR, Fraction, JSBI, ONE, TEN, Token, TokenAmount, TWO, ZERO } from '.';
import { CalculationError } from './errors';

export const FEE_DENOMINATOR = JSBI.BigInt(1e10);
...
export class StableSwap {
  ...
  public readonly balances: TokenAmount[]
  ...
  private _xp (balances: TokenAmount[], tokenMultipliers: JSBI[]): JSBI[] {
    return balances.map((balance, i) => JSBI.multiply(balance.raw, tokenMultipliers[i]));
  }
  ...
  private _getD (xp: JSBI[], amp: JSBI): JSBI {
    ...
    if (JSBI.equal(sum, ZERO)) return ZERO;
    ...
    for (let i = 0; i < 255; i++) {
      ...
      if (JSBI.lessThanOrEqual(this._distance(D, Dprev), ONE)) return D;
    }
  ...
  private _getY (inIndex: number, outIndex: number, inBalance: JSBI, normalizedBalances: JSBI[]): JSBI {
    ...
    for (let i = 0; i < 255; i++) {
      ...
      if (JSBI.lessThanOrEqual(this._distance(lastY, y), ONE)) return y;
    }
  ...
  private _getYD (A: JSBI, index: number, xp: JSBI[], D: JSBI): JSBI {
    ...
    for (let i = 0; i < 255; i++) {
      ...
      if (JSBI.lessThanOrEqual(this._distance(yPrev, y), ONE)) return y;
    }
  ...
  private get _feePerToken (): JSBI {
    ...
    return JSBI.divide(
      JSBI.multiply(this.fee, JSBI.BigInt(nCoins)),
  ...
  public getTokenIndex (token: Token): number {
    for (let i = 0; i < this.pooledTokens.length; i++) {
      if (token.equals(this.pooledTokens[i])) return i;
    }
  ...
  public calculateTokenAmount (amounts: TokenAmount[], deposit: boolean): TokenAmount {
    const nCoins = this.pooledTokens.length;
    ...
    if (JSBI.equal(totalSupply, ZERO)) return new TokenAmount(this.lpToken, D1);
    const diff = deposit ? JSBI.subtract(D1, D0) : JSBI.subtract(D0, D1);

    return new TokenAmount(
      this.lpToken,
      JSBI.divide(JSBI.multiply(diff, this.totalSupply.raw), D0)
    );
  }

  public calculateSwap (inIndex: number, outIndex: number, inAmount: TokenAmount): TokenAmount {
    const normalizedBalances = this.xp;
    const newInBalance = JSBI.add(
      normalizedBalances[inIndex],
      JSBI.multiply(inAmount.raw, this.tokenMultipliers[inIndex])
    );
    ...
    const fee = JSBI.divide(JSBI.multiply(this.fee, outAmount), FEE_DENOMINATOR);

    return new TokenAmount(this.pooledTokens[outIndex], JSBI.subtract(outAmount, fee));
  }

  public calculateRemoveLiquidity (amount: TokenAmount): TokenAmount[] {
    invariant(JSBI.lessThanOrEqual(amount.raw, this.totalSupply.raw), 'Cannot exceed total supply');

    return this.balances.map(
      (balance, i) => new TokenAmount(
        this.pooledTokens[i],
        JSBI.divide(JSBI.multiply(balance.raw, amount.raw), this.totalSupply.raw)
      )
    );
  }

  public calculateRemoveLiquidityOneToken (amount: TokenAmount, index: number): [TokenAmount, TokenAmount] {
    invariant(index < this.pooledTokens.length, 'indexOutOfRange');
    const amp = this.APrecise;
    const xp = this.xp;
    const D0 = this._getD(xp, amp);
    const D1 = JSBI.subtract(D0, JSBI.divide(JSBI.multiply(amount.raw, D0), this.totalSupply.raw));
    ...
    return [
      new TokenAmount(this.pooledTokens[index], dy),
      new TokenAmount(this.pooledTokens[index], fee)
    ];
  }
}
